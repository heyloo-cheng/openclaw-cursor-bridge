...
// 保存任务元数据（兼容旧工作流格式）
async function saveTaskMeta(task) {
    try {
        const meta = {
            task_name: task.id,
        ...
        await fs.writeFile(path.join(RESULT_DIR, "task-meta.json"), JSON.stringify(meta, null, 2));
    }
    catch (error) {
...
// 保存最新结果（兼容旧工作流格式）
async function saveLatestResult(task) {
    try {
        const result = {
        ...
        await fs.writeFile(path.join(RESULT_DIR, "latest.json"), JSON.stringify(result, null, 2));
    }
    catch (error) {
...
// 保存待唤醒通知（兼容旧工作流格式）
async function savePendingWake(task) {
    try {
        const wake = {
            task_name: task.id,
            feishu_target: task.feishuTarget || "",
        ...
        await fs.writeFile(path.join(RESULT_DIR, "pending-wake.json"), JSON.stringify(wake, null, 2));
    }
    catch (error) {
...
${summary}`;
        ...
        await execAsync(`openclaw message send --channel feishu --target "${task.feishuTarget}" --message "${message.replace(/"/g, '\\"')}"`);
